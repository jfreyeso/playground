    ...
    def set_default_model(self) -> None:
        ...
        if self.model is None:
            try:
                ...
                log_exception(e)
                log_error(
                    "Agno agents use `Claude` as the default model provider. "
                    "Please provide a `model` or install `anthopic`."
                )
                exit(1)

            log_info("Setting default model to OpenAI Chat")
            self.model = Claude(id="arn:aws:bedrock:us-east-2:204664655215:inference-profile/us.anthropic.claude-3-7-sonnet-20250219-v1:0")
